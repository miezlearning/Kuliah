...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($_FILES['user_image']['tmp_name'], $directory . $newImageName)) {
        ...
        $insert_query = "INSERT INTO posttest (username, password, user_image) VALUES ('$username', '$password', '$newImageName')";
...
if (isset($_POST['update'])) {
    ...
    if (isset($userImage) && $_FILES['user_image']['error'] === UPLOAD_ERR_OK) {
        $update_query = "UPDATE posttest SET username = '$newUsername', password = '$newPassword', user_image = '$userImage' WHERE id = $userId";
    } else {
